...
int assignMessage(Message& msg,vector<int>& route,int start_time,int end_time,map<pair<int,int>,set<int>>& S,int& departure_time){
    ...
    for(int rep = 0;rep < repeats[msg];rep++){
        ...
        for(int disjoint = 0;disjoint < M[msg].tl ;disjoint++){
            ...
            while(flag == 0){
                ...
                // print_vec(A);
                flag = assignMessage(M[msg],A,start_time,end_time,S,departure_time);
