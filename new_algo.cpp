...
AlgoResults my_holistic_algo(int num_ecu,int num_bridges,vector<Message> M,int Bridge_limit,int link_build_cost,int yens_kmax,int assignment_type,int verbose,int debug_print){
    ...
    for(int Holistic_idx = 0;Holistic_idx < Holistic_Order.size();Holistic_idx++){
        auto [msg,rep,start_time_,end_time_,message_] = Holistic_Order[Holistic_idx];
...
void new_algo_bind(py::module_ &m) {
    ...
    m.def("my_algo", [](int num_ecu, int num_bridges, vector<Message> M, 
    ...
    m.def("my_holistic_algo", [](int num_ecu, int num_bridges, vector<Message> M, 
                 int Bridge_limit, int link_build_cost,int yens_kmax, 
                 int assignment_type, int verbose, int debug_print) {
    ...
    return my_holistic_algo(num_ecu, num_bridges, M, Bridge_limit, 
                link_build_cost, yens_kmax, assignment_type, verbose, debug_print);
    },
    // py::return_value_policy::take_ownership,
    py::arg("num_ecu"),
